  ...
  <div>
    ...
    <div class="container">
      <TableCustom :columns="columns"
        ...
        <template #toolbarBtn>
          <h4>部门：{{deptName}}</h4>
          <el-button type="warning" :icon="CirclePlusFilled" @click="AddVisible = true">添加加班记录</el-button>
        </template>
...
import {deleteDeptOvertimeRecord, insertDeptOvertimeRecord, listDeptOvertimeRecord, updateDeptOvertimeRecord} from "@/api/forOvertimeRecord";
...
const employee_SV = ref<SelectionView[]>([]);
...
let columns = ref([
  { prop: 'id', label: '编号' },
  { prop: 'empId', label: '工号' },
  { prop: 'empId', label: '姓名', type: 'selection-view', selectionView: employee_SV },
...
const editData = async (form: OvertimeRecord) => {
  closeEditDialog()
  await updateDeptOvertimeRecord(form)
  getData();
};
...
const insertData = async (form: OvertimeRecord) => {
  ...
  closeAddDialog()
  await insertDeptOvertimeRecord(form)
  getData();
};
...
async function handleView(row: OvertimeRecord){
  ...
  viewData.value.list = [
    { prop: 'id', label: '编号' },
    { prop: 'empId', label: '工号' },
    { prop: 'empId', label: '姓名', isSelectionView: true, selectionView: employee_SV },
    { prop: 'start', label: '开始时间' },
    { prop: 'end', label: '结束时间' },
...
async function handleDelete(row: OvertimeRecord) {
  await deleteDeptOvertimeRecord({id: row.id})
  getData()
}
